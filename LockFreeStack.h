...
/**
 * This is my implementation of the push function for this lock free stack utilizing atomics
 ...
 * Since this function returns void, we do not need to return anything
 */
void
Stack::push(int val)
{
    ...
    bool succeeded = false;
    while(!succeeded) {
        newHead.link = new StackItem(val);
        newHead.link->next = expected.link;
        newHead.count = expected.count + 1;
        succeeded = head.compare_exchange_weak(expected, newHead);
        // this conditional is tripped if the compare_exchange_weak() evaluates to false
        // since the operation was not completed on this iteration of the loop, we need to delete the newHead.link to prevent memory leaks
        if (!succeeded) {
            delete newHead.link;
        }
    }
